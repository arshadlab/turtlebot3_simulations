...
def generate_launch_description():
    ...
    declare_rviz_config_file_cmd = DeclareLaunchArgument(
        ...
        default_value=os.path.join(
            bringup_dir, 'rviz', 'multi_nav2_default_view.rviz'),
    ...
    urdf = os.path.join(
        get_package_share_directory(
            'turtlebot3_description'), 'urdf', 'turtlebot3_' + TURTLEBOT3_MODEL + '.urdf'
    )

    world = os.path.join(
        get_package_share_directory('turtlebot3_gazebo'),
        'worlds', os.path.join('turtlebot3_worlds', 'multi_robot.model')
    )
    ...
    for robot in robots:
        ...
        spawn_turtlebot3_burger = Node(
            ...
            arguments=[
                '-file', os.path.join(turtlebot3_gazebo,'models', 'multi_turtlebot3_burger', 'model.sdf'),
                '-entity', robot['name'],
    ...
    for robot in robots:
        ...
        initial_pose_cmd = ExecuteProcess(
            cmd=['ros2', 'topic', 'pub', '-1', namespace + ['/initialpose'],
        ...
        post_turtlebot3_event = RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=last_action,
